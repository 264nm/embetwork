...
## designed to be run manually for debugging purposes. This script builds on the one released
## in shm 1.8.2 as pidchecker.sh. 

## It now serves the following features
...
_chkTraining()
{
  ...
  do
    ...
    then
      ...
      for file in /home/root/ml_training*
      do
...
_checkPatch()
{
  ...
  for file in /home/root/patches/*.patch
  do
    if test -e "$_blockPath"
    then
      break
    fi
...
echo "General Info: -------------------------------------------------------"
  echo "$_uptime since last boot"
  echo "Node software is $_version"
  _checkPatch
