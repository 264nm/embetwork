...
mainrepo="bridgemonitor.research.nicta.com.au"
...
## before packaging
/usr/bin/svn checkout https://github.inside.nicta.com.au/shm/shm-node/trunk/packages/ipkg/"$package" > /tmp/fetchout.log
...
if test -f ./ipkg-build.sh; then
  ...
  for x in `/usr/bin/md5sum ./ipkg-build.sh | awk '{ print $1 }'`; do
    if [ "$x" == "090cd528cb2a863b85bbb04d1b7d76a6" ]; then
      ...
      echo -e "Try fetching manually from: \nhttps://bridgemonitor.research.nicta.com.au/opkg/ipkg-build.sh" 
    fi
  done
else 
