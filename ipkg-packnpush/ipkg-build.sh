...
# "dpkg-scanpackages -t ipk" cannot process tgz, need ar format only
# Also, as of dpkg-scanpackages v1.17.5 in ubuntu 14.04, scanpackages
# is validating the order of files in the ar archive; per 'man 5 deb',
# the order is: debian-binary,control.tar, data.tar
# It has always been like that, but scanpackages only just started checking it.
# dpkg-scanpackages v1.16.1.2 in ubuntu 12.04 didn't do that check.
cd $tmp_dir
ar -rcv ../$pkg_file ./debian-binary ./control.tar.gz ./data.tar.gz
cd ..
